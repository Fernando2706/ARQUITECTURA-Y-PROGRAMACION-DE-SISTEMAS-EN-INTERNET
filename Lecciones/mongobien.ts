...
try{
  ...
  const data = await Promise.all(dataPromises); //Se espera a que se cumplan todas las promesas

  const results = {
    characters: data[0],
    episodes: data[1],
    locations: data[2],
  }//Se guarda la refencia de cada elemento de data

  const DB_URL = Deno.env.get("DB_URL");
  const DB_NAME = Deno.env.get("DB_NAME");

  //Para no subir datos secretos a github se usan las variables de entorno
  ...
  client.connectWithUri(DB_URL);//Se conecta a la base de datos
  const db = client.database(DB_NAME);//Se conecta a la base de datos en concreto
